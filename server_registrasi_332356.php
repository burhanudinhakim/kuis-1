<?php
  ...
  $server->register('login_ws', 
    ...
    function login_ws($username, $password) { //enkripsi password dengan md5 $password = md5($password);
      ...
      $db -> Connect('128.199.182.167','kuis1','rahasia',''); //cek username dan password dari database
	...
	function simpan_data() {
		$db = NewADOConnection('mysql');
		$db -> Connect('128.199.182.167','kuis1','rahasia','');
		...
		$sql = $db -> Execute("INSERT INTO pengguna(email,nim,nama,jenis_kelamin, alamat)");
