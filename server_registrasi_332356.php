<?php
  ...
  $server->register('register_ws', 
    array(
		...
		'alamat'		 =>'xsd:string'), //parameters dari register ws yang ada di client
      array(
        'return' => 'xsd: string'
      ), //output 
    ...
    function register_ws($email, $nim, $nama, $jenis_kelamin, $alamat) { //enkripsi password dengan md5 $password = md5($password);
	  ...
	  //cek ke database lihat fungsi mysql di database ciount (ditambah satu fungsi)
      ...
      if ($sql->RecordCount() >= 1 || $sql2->RecordCount() >= 1) //sama dengan mysql_num_rows pada php biasa
      {
		...
		$sql3 =$db->Execute("insert into pengguna (email, nim, nama, jenis_kelamin, alamat) 
		values ('$email', '$nim', '$nama', '$jenis_kelamin', '$alamat')");
